...
class W {
  ...
  #R() {
    ...
    for (let i = 1; i < e.count; i++) {
      const s = 3 * i;
      t[s] = E(2 * e.maxX); // Full X spread
      
      // Instead of an explosive center cluster, spawn them in the upper half 
      // of the screen and high above it, so they elegantly cascade down like a waterfall
      t[s + 1] = Math.random() * (e.maxY * 3); 
  ...
  update(e) {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      if (t.controlSphere0) {
        ...
        if (dist < sumRadius0) {
          const diff = sumRadius0 - dist; j.copy(_.normalize()).multiplyScalar(diff);
          H.copy(j).multiplyScalar(Math.max(B.length(), 2)); I.sub(j); B.sub(H);
        }
      }
...
const X = {
  ...
  materialParams: { metalness: 0.1, roughness: 0.3, clearcoat: 1, clearcoatRoughness: 0.1 },
...
class Z extends m {
  constructor(e, t = {}) {
    ...
    // Bottom Half (Inner Gel Body)
    const o1 = new g(0.65, 0.8, 16, 32); 
    const r1 = new c({ envMap: n, ...i.materialParams });
    r1.envMapRotation.x = -Math.PI / 2;
    this.mesh1 = new d(o1, r1, i.count);
    
    // Top Half (Outer Gel Cap)
    // Slightly wider, shorter, and will be shifted to overlap the inner body to create a physical seam
    const o2 = new g(0.68, 0.2, 16, 32);
    const r2 = new c({ envMap: n, ...i.materialParams });
    r2.envMapRotation.x = -Math.PI / 2;
    this.mesh2 = new d(o2, r2, i.count);
    ...
    // Initialize random rotations and tumble velocities
    this.rotations = [];
    this.rotVels = [];
    for(let k=0; k<i.count; k++) {
        this.rotations.push(new euler(Math.random()*Math.PI*2, Math.random()*Math.PI*2, Math.random()*Math.PI*2));
        this.rotVels.push(new euler((Math.random()-0.5)*0.03, (Math.random()-0.5)*0.03, (Math.random()-0.5)*0.03));
    }
  }
  #S() {
  ...
  setColors(e) {
    if (Array.isArray(e) && e.length > 1) {
      ...
      const whiteColor = new l(0xF8F9FA); // Premium bright clinical white
      
      for (let idx = 0; idx < this.count; idx++) {
        const gradColor = t.getColorAt(idx / this.count);
        
        // Alternate creating colored caps on white bodies, and white caps on colored bodies!
        if (idx % 2 === 0) {
            this.mesh1.setColorAt(idx, gradColor);
            this.mesh2.setColorAt(idx, whiteColor);
        } else {
            this.mesh1.setColorAt(idx, whiteColor);
            this.mesh2.setColorAt(idx, gradColor);
        }
        
        if (idx === 0) { this.light.color.copy(gradColor); }
      }
      this.mesh1.instanceColor.needsUpdate = true;
  ...
  update(e) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      // Update Rotations to create tumbling pills
      this.rotations[idx].x += this.rotVels[idx].x;
      this.rotations[idx].y += this.rotVels[idx].y;
      this.rotations[idx].z += this.rotVels[idx].z;
      
      let scale = this.physics.sizeData[idx];
      ...
      U.updateMatrix();
      this.mesh1.setMatrixAt(idx, U.matrix);
      
      // 2. Render Outer Cap (Shifted along local Y axis to overlap)
      const oY = U.position.y, oX = U.position.x, oZ = U.position.z;
      U.translateY(0.3 * scale); // Shift cap to perfectly envelop the top half of the inner body
      U.updateMatrix();
      this.mesh2.setMatrixAt(idx, U.matrix);
      
      // Restore position for any subsequent physics logic or lighting calculation
      U.position.set(oX, oY, oZ);

      if (idx === 0) this.light.position.copy(U.position);
    }
    this.mesh1.instanceMatrix.needsUpdate = true;
...
function createBallpit(e, t = {}) {
  ...
  // Inject true screen world size into the initial spawn so pills don't cluster in the middle
  ...
  return {
    three: i, get spheres() { return s; },
    setCount(e) { initialize({ ...s.config, count: e }); },
    updateConfig(newProps) {
      if (newProps.count !== undefined && newProps.count !== s.config.count) { initialize({ ...s.config, ...newProps }); } else {
        Object.assign(s.config, newProps);
        if (newProps.colors) { s.setColors(s.config.colors); }
        if (newProps.minSize !== undefined || newProps.maxSize !== undefined || newProps.size0 !== undefined) { s.physics.setSizes(); }
      }
    },
    togglePause() { c = !c; }, dispose() { h.dispose(); i.dispose(); }
  };
}

const Pillpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    // Create a fresh canvas to prevent WebGL context loss errors in React Strict Mode
  ...
  useEffect(() => {
    ...
    if (spheresInstanceRef.current) { spheresInstanceRef.current.updateConfig({ followCursor, ...props }); }
  }, [props, followCursor]);

  return <div className={className} ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative' }} />;
};

export default Pillpit;
