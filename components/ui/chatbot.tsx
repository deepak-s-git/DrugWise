...
export function Chatbot() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (window.scrollY > window.innerHeight * 0.8) {
        ...
        setIsVisible(false);
        setIsOpen(false); // Auto-close if they scroll back up to hero
      }
    };
  ...
  return (
    <div className={`transition-opacity duration-500 z-50 ${isVisible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      <motion.button
